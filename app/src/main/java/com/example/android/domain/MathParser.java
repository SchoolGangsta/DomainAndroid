...
public class MathParser {
    ...
    public  static ArrayList<String> solvable = new ArrayList<>();
    public MathParser() {
        var = new HashMap<>();
    ...
    public double Parse(String s, double x) throws Exception {
        if(s.isEmpty())
            throw new Exception("Empty expression");
        replaceVariable("x", x);
        Result result = binaryFunc(s);
        ...
        while(true){
            ...
            switch(sign){
                case '*':
                    ...
                    break;
                case '/':
                    String tmp = next.substring(0, next.length()-right.rest.length());
                    if(!solvable.contains(tmp))
                        solvable.add(tmp);
                    acc/=right.acc;
                    break;
